#!/usr/bin/env python3
"""Moving Fibre Intelligence L3 v1.8.1 shared category-conditioned writer audit."""
...
def _description_features(model,tok,nodes,device):
    ...
    batch=tok(prompts,return_tensors="pt",padding=True,truncation=True,max_length=48).to(device)
    out=model(**batch,output_hidden_states=True)
    last=batch["attention_mask"].sum(1)-1; rows=torch.arange(len(prompts),device=device)
    h=torch.nn.functional.normalize(out.hidden_states[-1][rows,last],dim=-1)
...
def _fit_shared_router(features, fit_indices, fit_labels, classes, ridge):
    ...
    pred=[classes[int(i)] for i in scores.argmax(1)]
    return pred,scores
...
def one_seed_l3_category_geometry(cfg,seed,output):
    ...
    features=_description_features(model,tok,L3_NODES,device)
    classes=sorted({n["category"] for n in L3_NODES})
    ...
    for idx,node in enumerate(L3_NODES):
        ...
        result.update({"concept":node["name"],"description":node["description"],
            ...
            "minimum_heldout_signed_margin":min(result["paraphrase_signed_margins"])})
        runs.append(result)
    labels=[n["category"] for n in L3_NODES]
    train_idx=[i for i,n in enumerate(L3_NODES) if not n.get("heldout")]
    held_idx=[i for i,n in enumerate(L3_NODES) if n.get("heldout")]
    ...
    for arm,(fit_idx,fit_labels) in router_specs.items():
        predictions,scores=_fit_shared_router(features,fit_idx,fit_labels,classes,cfg["category_router_ridge"])
        moved=[]; movement=[]; access=[]
        for i,node in enumerate(L3_NODES):
            endpoint,drift,kl,scale,halves=_move_on_category_direction(
                model,tok,params,cfg,anchors,response_ids,response0,logits0,
                endpoints[i],directions[predictions[i]],device)
            ...
            row={"concept":node["name"],"truth":labels[i],"router_prediction":predictions[i],
                 "router_correct":predictions[i]==labels[i],"category_move_scale":scale,
        ...
        metric=_category_metrics(np.stack(moved),labels,train_idx,train_labels,train_idx,held_idx)
    ...
    no_graph=_category_metrics(np.stack(raw_positions),labels,train_idx,train_labels,train_idx,held_idx)
    ...
    record={"seed":seed,"device":str(device),"symbols":[sym0,sym1],"anchors":anchors,
        ...
        "train_concepts":[L3_NODES[i]["name"] for i in train_idx],
        "heldout_concepts":[L3_NODES[i]["name"] for i in held_idx],
        "shared_router":"ridge-on-frozen-description-features",
        "category_move_scale":cfg["category_move_scale"]},"concept_runs":runs,"category_arms":arms,
