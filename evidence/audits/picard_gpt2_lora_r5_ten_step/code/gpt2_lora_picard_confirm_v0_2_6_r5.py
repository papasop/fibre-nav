...
PROTOCOL="GPT2_LORA_PICARD_V0_2_6_R5_TEN_STEP_RESOLUTION_AUDIT"
...
def run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,model_name,device,kind,lr,alpha,seed,steps,batch_size,target,deadline,eval_every=EVAL_EVERY):
...
def main():
 ...
 print(f"[frozen] no pilot; adamw_lr={ADAM_LR} picard_alpha={PICARD_ALPHA} picard_lr={PICARD_LR} target={FROZEN_TARGET:.12f} eval_every={EVAL_EVERY}",flush=True)
 ...
 gates={"no_same_run_hyperparameter_tuning":True,"ten_step_target_detection":EVAL_EVERY==10,"frozen_dataset_sha256_matches":data_identity["sha256"]==DATA_SHA256,"real_gpt2_lora_model":True,"same_lora_parameterization":True,"shared_warm_start_identical":all(abs(p["shared_warm_val_loss_delta"])<=1e-10 for p in pairs),"same_five_seeds_as_r4_resolution_control":EVAL_SEEDS==[23311,23321,23327,23333,23339],"all_picard_runs_stable":all(next(r for r in records if r["seed"]==s and r["kind"]=="picard_a05")["stable"] for s in EVAL_SEEDS),"both_arms_reach_frozen_target_all_seeds":valid,"median_time_to_equal_loss_speedup_at_least_10pct":valid and eq>=.10,"at_least_four_of_five_seed_speedups_positive":positive>=4,"median_final_loss_delta_at_most_0_02":loss_delta<=.02,"each_final_loss_delta_at_most_0_03":all(p["final_val_loss_delta"]<=.03 for p in pairs),"runtime_within_hard_limit":time.monotonic()<=deadline};supported=all(gates.values())
 summary={"protocol":PROTOCOL,"mode":"frozen_ten_step_resolution_audit","model":a.model,"data_identity":data_identity,"sequence_length":a.seq_len,"batch_size":a.batch_size,"target_evaluation_interval_steps":EVAL_EVERY,"shared_warm_start":{"steps":WARM_STEPS,"optimizer":"AdamW","lr":WARM_LR,"excluded_from_comparison_timing":True},"frozen_configuration":{"adamw_lr":ADAM_LR,"picard_lr":PICARD_LR,"picard_metric_exponent":PICARD_ALPHA,"target_val_loss":FROZEN_TARGET,"source_protocol":"GPT2_LORA_PICARD_V0_2_6_R4_FROZEN_CONFIRMATORY"},"eval_seeds":EVAL_SEEDS,"pairs":pairs,"positive_seed_count":positive,"median_time_to_equal_loss_speedup_fraction":eq,"median_fixed_budget_speedup_fraction_diagnostic":fixed,"median_final_val_loss_delta":loss_delta,"wall_seconds":time.monotonic()-start,"gates":gates,"scientific_status":"GPT2_LORA_PICARD_V0_2_6_R5_TEN_STEP_RESOLUTION_SUPPORTED" if supported else "GPT2_LORA_PICARD_V0_2_6_R5_TEN_STEP_RESOLUTION_INCONCLUSIVE_FAIL_CLOSED","claim_boundary":"Measurement-resolution audit of the frozen r4 GPT-2-small, rank-4 c_attn LoRA result using the same five seeds but target checks every 10 rather than 50 steps. Shared warm start is excluded from timing; fixed-budget speed is diagnostic. This is not an independent new-seed replication or a universal optimizer claim."}
