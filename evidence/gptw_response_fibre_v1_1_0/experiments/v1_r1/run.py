#!/usr/bin/env python3
"""GPTW-v1-r1: dependency-robust GPT-2/native-LoRA response-fibre audit."""
...
PROTOCOL="CNER_GPT2_LORA_RESPONSE_FIBRE_ADAPTIVE_VALUE_V1_R1"
MODEL_IDS=["openai-community/gpt2","gpt2"]
SEEDS=[24742,24743,24744,24745,24746,24747,24748,24749]
REQUIRED=6; RANDOM_CONTROLS=16; LORA_RANK=2; LAYERS=[10,11]
ANCHOR_RESPONSE_DIM=16; UPDATE_NORM=0.35; FD_EPS=1e-3
MAX_RELATIVE_ANCHOR_LEAKAGE=0.025; MAX_RANDOM_LEAKAGE_MISMATCH=0.015
MIN_GAIN_MARGIN=1e-4; MAX_FD_JVP_RELERR=0.08; WALL_CLOCK_LIMIT_SECONDS=3300
...
def project_kernel(v,J):
    ...
    coeff=torch.linalg.solve(gram+(1e-8*max(scale,1e-12))*torch.eye(len(gram),device=J.device),J@v)
    return v-J.T@coeff
...
def run_seed(seed,device):
    ...
    base_loss=float(task_loss(model,tok,HELDOUT,device)); selected=project_kernel(-grad_flat(task_loss(model,tok,CONSTRUCTION,device),params),J)
    ...
    for _ in range(RANDOM_CONTROLS):
        q=project_kernel(torch.randn(len(base_vec),generator=gen,device=device),J)
    ...
    # Directional derivative / symmetric finite-difference certification of the frozen response Jacobian.
    u=unit(selected); jvp=J@u
    assign(params,base_vec+FD_EPS*u); rp=response(model,tok,device).detach()
    assign(params,base_vec-FD_EPS*u); rm=response(model,tok,device).detach(); assign(params,base_vec)
    fd=(rp-rm)/(2*FD_EPS); fd_rel=float((fd-jvp).norm()/jvp.norm().clamp_min(1e-12))
    ...
    gates={"beats_no_tangent":selected_gain>=MIN_GAIN_MARGIN,
           ...
           "fd_jvp_consistency":fd_rel<=MAX_FD_JVP_RELERR,
    ...
    out={"seed":seed,"model":model_id,"lora_implementation":"native Conv1D wrapper with frozen random A and audited B coordinates","lora_rank":LORA_RANK,"lora_layers":LAYERS,"trainable_parameters":len(base_vec),
         ...
         "finite_difference_jvp_relative_error":fd_rel,"projected_kernel_relative_residual":kernel_rel,
         "gates":gates,"instance_supported":all(gates.values())}
    del model; torch.cuda.empty_cache(); return out

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--output",default="cner_gpt2_lora_response_fibre_adaptive_value_v1_r1_results"); args,_=ap.parse_known_args()
    if not torch.cuda.is_available(): raise RuntimeError("GPTW-v1-r1 requires CUDA; select an A100 runtime")
    ...
    for i,s in enumerate(SEEDS):
        if time.monotonic()-started>WALL_CLOCK_LIMIT_SECONDS: raise TimeoutError("GPTW-v1-r1 exceeded its 55-minute hard limit")
        print(f"[GPTW-v1-r1 {i+1}/8] seed={s}",flush=True); row=run_seed(s,device); rows.append(row)
    ...
    report={"protocol":PROTOCOL,"scientific_status":"PROSPECTIVE_GPT2_LORA_RESPONSE_FIBRE_AUDIT_COMPLETED",
      "decision":"GPT2_LORA_SELECTED_RESPONSE_FIBRE_VALUE_SUPPORTED" if passed else "GPT2_LORA_SELECTED_RESPONSE_FIBRE_VALUE_NOT_SUPPORTED",
      ...
      "maximum_random_leakage_mismatch":MAX_RANDOM_LEAKAGE_MISMATCH,"maximum_fd_jvp_relative_error":MAX_FD_JVP_RELERR},
      "instance_summaries":[{k:x[k] for k in ("seed","base_heldout_loss","selected","sign_reversed","shuffled_target","best_of_16_random","finite_difference_jvp_relative_error","projected_kernel_relative_residual","gates","instance_supported")} for x in rows],
      "claim_boundary":"Frozen GPT-2 with LoRA only in attention c_attn modules of layers 10-11; finite built-in prompt task and declared anchor-logit response. A pass supports selected response-fibre tangent value in this LoRA domain only; it is not moving-fibre transport, a full-model claim, semantic generality, an action principle, or global minimality."}
