#!/usr/bin/env python3
"""GPTW-v1-r2a: dtype-corrected same-seed float64 derivative precision repair."""
...
PROTOCOL="CNER_GPT2_LORA_RESPONSE_FIBRE_ADAPTIVE_VALUE_V1_R2A_PRECISION"
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--output",default="cner_gpt2_lora_response_fibre_adaptive_value_v1_r2a_precision_results"); args,_=ap.parse_known_args()
    if not torch.cuda.is_available(): raise RuntimeError("GPTW-v1-r2a requires CUDA; select an A100 runtime")
    ...
    for i,s in enumerate(SEEDS):
        if time.monotonic()-started>WALL_CLOCK_LIMIT_SECONDS: raise TimeoutError("GPTW-v1-r2a exceeded its 55-minute hard limit")
        print(f"[GPTW-v1-r2a precision {i+1}/8] seed={s}",flush=True); row=run_seed(s,device); rows.append(row)
    ...
    report={"protocol":PROTOCOL,"scientific_status":"SAME_SEED_DERIVATIVE_PRECISION_REPAIR_COMPLETED",
      "decision":"GPT2_LORA_DERIVATIVE_PRECISION_REPAIR_SUPPORTED" if passed else "GPT2_LORA_DERIVATIVE_PRECISION_REPAIR_NOT_SUPPORTED",
      ...
      "claim_boundary":"Same-seed float64 numerical repair of the v1-r1 kernel-direction finite-difference gate. A pass validates the derivative audit and preserves the observed effect pattern, but is not a prospective new-seed confirmation, moving-fibre transport, a full-model claim, an action principle, or global minimality."}
