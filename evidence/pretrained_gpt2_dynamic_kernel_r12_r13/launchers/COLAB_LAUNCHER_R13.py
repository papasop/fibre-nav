...
if os.path.exists("/content"):
 ...
 print("Upload picard_gpt2_pretrained_dynamic_r13_confirm.zip",flush=True); z=next(iter(files.upload()))
else: z="picard_gpt2_pretrained_dynamic_r13_confirm.zip"
root=Path("/content/r13_run" if os.path.exists("/content") else "r13_run"); root.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(z) as f:f.extractall(root)
scripts=list(root.rglob("picard_gpt2_pretrained_r13_confirm.py")); assert len(scripts)==1,scripts
out=Path("/content/picard_r13_results" if os.path.exists("/content") else "picard_r13_results")
