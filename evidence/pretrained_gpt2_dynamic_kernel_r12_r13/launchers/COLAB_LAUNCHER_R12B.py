...
if os.path.exists("/content"):
 ...
 print("Upload picard_gpt2_pretrained_dynamic_r12b.zip",flush=True); z=next(iter(files.upload()))
else: z="picard_gpt2_pretrained_dynamic_r12b.zip"
root=Path("/content/r12b_run" if os.path.exists("/content") else "r12b_run"); root.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(z) as f:f.extractall(root)
scripts=list(root.rglob("picard_gpt2_pretrained_r12b.py")); assert len(scripts)==1,scripts
out=Path("/content/picard_r12b_results" if os.path.exists("/content") else "picard_r12b_results")
