#!/usr/bin/env python3
"""Robust Colab launcher for Moving Fibre Intelligence L3 v1.9.1."""
import argparse
import os
import shutil
import subprocess
...
def choose_zip(requested, explicit):
    ...
    try:
        from google.colab import files
        print("Please select moving_fibre_intelligence_l3_v1_9_1.zip")
...
def main():
    ...
    p.add_argument("--zip", default="/content/moving_fibre_intelligence_l3_v1_9_1.zip")
    p.add_argument("--workdir", default="/content/moving_fibre_intelligence_l3_run_v1_9_1")
    ...
    print("[MFI L3 v1.9.1 prospective confirmation] frozen fresh seed: 82101", flush=True)
    code = stream(cmd, root, log_path)

    archive = Path(shutil.make_archive("/content/moving_fibre_intelligence_l3_results_v1_9_1", "zip", results))
    if code != 0:
