...
PROTOCOL="GPT2_LORA_CACHED_PICARD_V0_2_6_R2_WARM_CHART_DEVELOPMENT"
PILOT_SEED=23063
...
def run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,model_name,device,kind,lr,seed,steps,batch_size,target,deadline,eval_every=50):
 ...
 for step in range(steps):
  ...
  if kind=="adamw":opt.step()
   ...
   with torch.no_grad():
    grads=[p.grad.detach() for p in params];direction=torch._foreach_add(grads,params,alpha=.001);direction=torch._foreach_div(direction,metric);torch._foreach_mul_(mom,.85);torch._foreach_add_(mom,direction,alpha=.15);torch._foreach_add_(params,mom,alpha=-lr)
...
def main():
 ...
 AdamGrid=[2e-4,5e-4];PicardGrid=[.01,.03,.1,.3];pilot={};chosen={};selected={}
 pilot_records=[]
 for kind,grid in (("adamw",AdamGrid),("picard",PicardGrid)):
  cand=[]
  for lr in grid:
   print(f"[pilot] {kind} lr={lr}",flush=True);r=run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,a.model,device,kind,lr,PILOT_SEED,a.pilot_steps,a.batch_size,-math.inf,deadline,max(25,a.pilot_steps//2));pilot_records.append(r);finite=math.isfinite(r["final_val_loss"]);cand.append((not finite,r["final_val_loss"] if finite else math.inf,lr,r));print(f"[pilot done] finite={finite} loss={r['final_val_loss']:.6f}",flush=True)
  failed,score,lr,r=min(cand,key=lambda q:q[:3])
  if failed:raise RuntimeError(f"no finite {kind} pilot candidate")
  chosen[kind]=lr;selected[kind]=r;pilot[kind]={"lr":lr,"final_val_loss":score,"grid":grid}
 ...
 gates={"real_gpt2_lora_model":True,"same_lora_parameterization":True,"shared_warm_start_identical":all(abs(p["shared_warm_val_loss_delta"])<=1e-10 for p in pairs),"warm_chart_after_50_shared_steps":WARM_STEPS==50,"fused_foreach_picard_update":True,"two_new_eval_seeds":len(pairs)==2,"both_arms_reach_development_target":valid,"median_time_to_equal_loss_speedup_positive":valid and eq>0,"median_fixed_budget_speedup_positive":fixed>0,"median_final_loss_delta_at_most_0_02":loss_delta<=.02,"runtime_within_hard_limit":time.monotonic()<=deadline};signal=all(gates.values())
 summary={"protocol":PROTOCOL,"mode":"warm_chart_quick_development_nonclaim","model":a.model,"data_source":data_source,"sequence_length":a.seq_len,"batch_size":a.batch_size,"shared_warm_start":{"steps":WARM_STEPS,"optimizer":"AdamW","lr":WARM_LR,"excluded_from_comparison_timing":True},"pilot":pilot,"development_target":target,"eval_seeds":EVAL_SEEDS,"pairs":pairs,"median_time_to_equal_loss_speedup_fraction":eq,"median_fixed_budget_speedup_fraction":fixed,"median_final_val_loss_delta":loss_delta,"wall_seconds":time.monotonic()-start,"gates":gates,"scientific_status":"GPT2_LORA_PICARD_V0_2_6_R2_WARM_CHART_POSITIVE_SIGNAL" if signal else "GPT2_LORA_PICARD_V0_2_6_R2_WARM_CHART_INCONCLUSIVE_FAIL_CLOSED","claim_boundary":"Development-only GPT-2 small LoRA warm-chart mechanism test on WikiText-2 (or logged public-domain fallback); shared 50-step AdamW warm-start is excluded from comparison timing; two evaluation seeds after same-run pilot; not confirmatory, universal, or paper-level evidence."}
