...
PROTOCOL="GPT2_LORA_CACHED_PICARD_V0_2_6_R1_QUICK_DEVELOPMENT"
PILOT_SEED=23063
EVAL_SEEDS=[23117,23131]
...
def frozen_metric(model,params,train_blocks,device,torch,dtype,batch_size,seed):
 ...
 for f in fisher:
  scale=f.mean().clamp_min(1e-20);m=(f/scale+.1).to(dtype=f.dtype);metric.append(m)
 vals=torch.cat([m.flatten() for m in metric]);return metric,{"min":float(vals.min()),"median":float(vals.median()),"max":float(vals.max())}

def run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,model_name,device,kind,lr,seed,steps,batch_size,target,deadline,eval_every=50):
 ...
 order=schedule(torch,len(train_blocks),steps,batch_size,seed+17);mom=[torch.zeros_like(p,dtype=torch.float32) for p in params]
 ...
 sync(torch,device);timed=0.;hit=None;hit_step=None;trace=[]
 for step in range(steps):
  ...
  if kind=="adamw":opt.step()
   ...
   with torch.no_grad():
    for p,m,g in zip(params,mom,metric):
     direction=(p.grad.detach().float()+.001*p.detach().float())/g;m.mul_(.85).add_(direction,alpha=.15);p.add_(m.to(p.dtype),alpha=-lr)
  sync(torch,device);timed+=time.perf_counter()-t
  if (step+1)%eval_every==0 or step==steps-1:
   vl=evaluate(model,val_blocks,device,torch,dtype);trace.append({"step":step+1,"train_kernel_seconds":timed,"val_loss":vl})
 ...
 rec={"kind":kind,"seed":seed,"lr":lr,"steps":steps,"time_to_target":hit,"steps_to_target":hit_step,"timed_train_seconds":timed,"final_val_loss":final_loss,"metric_stats":metric_stats,"trainable_parameters":sum(p.numel() for p in params),"trace":trace}
 del opt,mom,metric,params,model;gc.collect()
...
def main():
 ...
 AdamGrid=[2e-4,5e-4];PicardGrid=[1e-3,3e-3,1e-2];pilot={};chosen={};selected={}
 pilot_records=[]
 for kind,grid in (("adamw",AdamGrid),("picard",PicardGrid)):
  cand=[]
  for lr in grid:
   print(f"[pilot] {kind} lr={lr}",flush=True);r=run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,a.model,device,kind,lr,PILOT_SEED,a.pilot_steps,a.batch_size,-math.inf,deadline,max(25,a.pilot_steps//2));pilot_records.append(r);cand.append((r["final_val_loss"],lr,r));print(f"[pilot done] loss={r['final_val_loss']:.6f}",flush=True)
  score,lr,r=min(cand,key=lambda q:q[0]);chosen[kind]=lr;selected[kind]=r;pilot[kind]={"lr":lr,"final_val_loss":score,"grid":grid}
 target=selected["adamw"]["final_val_loss"]
 print(f"[freeze-development] lr={chosen} target={target:.6f}",flush=True)
 out=Path(a.outdir);out.mkdir(parents=True,exist_ok=True);records=[]
 for i,seed in enumerate(EVAL_SEEDS,1):
  for kind in ("adamw","picard"):
   print(f"[eval {i}/{len(EVAL_SEEDS)}] seed={seed} {kind}",flush=True);r=run_arm(torch,AutoModelForCausalLM,LoraConfig,get_peft_model,train_blocks,val_blocks,a.model,device,kind,chosen[kind],seed,a.steps,a.batch_size,target,deadline);records.append(r);(out/f"{kind}_{seed}.json").write_text(json.dumps(r,indent=2)+"\n")
 pairs=[]
 for seed in EVAL_SEEDS:
  aa=next(r for r in records if r["seed"]==seed and r["kind"]=="adamw");pp=next(r for r in records if r["seed"]==seed and r["kind"]=="picard");valid=aa["time_to_target"] is not None and pp["time_to_target"] is not None
  pairs.append({"seed":seed,"valid":valid,"time_to_equal_loss_speedup_fraction":(aa["time_to_target"]-pp["time_to_target"])/aa["time_to_target"] if valid else None,"fixed_budget_speedup_fraction":(aa["timed_train_seconds"]-pp["timed_train_seconds"])/aa["timed_train_seconds"],"steps_reduction_fraction":(aa["steps_to_target"]-pp["steps_to_target"])/aa["steps_to_target"] if valid else None,"final_val_loss_delta":pp["final_val_loss"]-aa["final_val_loss"]})
 ...
 gates={"real_gpt2_lora_model":True,"same_lora_parameterization":True,"two_new_eval_seeds":len(pairs)==2,"both_arms_reach_development_target":valid,"median_time_to_equal_loss_speedup_positive":valid and eq>0,"median_fixed_budget_speedup_positive":fixed>0,"median_final_loss_delta_at_most_0_02":loss_delta<=.02,"runtime_within_hard_limit":time.monotonic()<=deadline};signal=all(gates.values())
 summary={"protocol":PROTOCOL,"mode":"quick_development_nonclaim","model":a.model,"data_source":data_source,"sequence_length":a.seq_len,"batch_size":a.batch_size,"pilot":pilot,"development_target":target,"eval_seeds":EVAL_SEEDS,"pairs":pairs,"median_time_to_equal_loss_speedup_fraction":eq,"median_fixed_budget_speedup_fraction":fixed,"median_final_val_loss_delta":loss_delta,"wall_seconds":time.monotonic()-start,"gates":gates,"scientific_status":"GPT2_LORA_PICARD_V0_2_6_R1_QUICK_POSITIVE_SIGNAL" if signal else "GPT2_LORA_PICARD_V0_2_6_R1_QUICK_INCONCLUSIVE_FAIL_CLOSED","claim_boundary":"Development-only GPT-2 small LoRA smoke comparison on WikiText-2 (or logged public-domain fallback); two evaluation seeds after same-run pilot; not confirmatory, universal, or paper-level evidence."}
