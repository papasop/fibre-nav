#!/usr/bin/env python3
"""Online moving-response-fibre preflight for the restricted F16 programme."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_MOVING_FIBRE_QUICK_V3_1A"
    seeds: int = 4
    base_seed: int = 66726
    adapt_steps: int = 96
    capability_loss_reduction_fraction: float = 0.10
    executable_step_radius: float = 0.12
    ...
    wrong_metric_natural_win_ceiling: int = 1
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_MOVING_FIBRE_QUICK_V3_1A"
        and p.seeds == 4
        and p.base_seed == 66726
        and p.adapt_steps == 96
        and p.capability_loss_reduction_fraction == 0.10
        and p.minimum_comparable_seeds == 3
        and p.success_required == 3
        and p.response_retraction_iterations == 2
    )
    if not locked:
        raise ValueError("v3.1a frozen quick protocol violated")
    return p
...
def row_space_projector(J, p, torch):
    ...
    return project, rank, J.shape[1] - rank, float(threshold)
...
def moving_kernel_basis(J, reference, p, torch):
    project, rank, null_dim, threshold = row_space_projector(J, p, torch)
...
def execute_online_path(name, theta0, basis0, target_response, batches, target_loss, model, spec,
                        anchor_x, cap_x, cap_y, metric_x, dev_x, dev_y, source_scales, p, deps):
    torch, _, F, functional_call = deps[:4]
    ...
    for step in range(p.adapt_steps):
        ...
        if candidate_cap <= target_loss:
            ...
            endpoint_alpha = 1.0
        new_basis, chart = moving_kernel_basis(J_new, basis, p, torch)
    ...
    admissible = bool(
        hit
        and max_kernel <= p.moving_kernel_residual_gate
    ...
    return {
        ...
        "fibre_rotation_signal": max_angle_source >= p.fibre_rotation_signal_gate_rad,
...
def summarize(rows, p):
    ...
    if n < p.minimum_comparable_seeds:
        status = "MOVING_FIBRE_V31A_INADMISSIBLE"
    elif gate:
        status = "MOVING_FIBRE_V31A_QUICK_CANDIDATE_SUPPORTED"
    else:
        status = "MOVING_FIBRE_V31A_QUICK_CANDIDATE_NOT_SUPPORTED"
    return {
        ...
        "counts": counts,
        "moving_fibre_candidate_gate": gate,
        "excluded": exclusions,
        "per_seed": per_seed,
        "claim_boundary": (
            "Four-seed development preflight of six online paths whose local directions, "
            "Fisher pullbacks, and capacity dual norms are recomputed from the current state. "
            "A Newton response retraction approximates finite constrained motion. This is not "
            "a 16-seed confirmation, exact continuum horizontal lift, arbitrary-path result, "
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_moving_fibre_quick_v3_1a_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Moving-fibre v3.1a seed {index + 1}/{p.seeds}] {seed}", flush=True)
    ...
    (args.output / "REPORT.md").write_text(
        "# Moving-response-fibre quick audit v3.1a\n\n```json\n"
