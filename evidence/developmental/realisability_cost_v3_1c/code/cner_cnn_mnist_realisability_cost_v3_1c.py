#!/usr/bin/env python3
"""Eight-seed realizability-cost and step-scaling audit for moving fibres."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_REALISABILITY_COST_V3_1C"
    seeds: int = 8
    base_seed: int = 68726
    adapt_steps: int = 384
    capability_loss_reduction_fraction: float = 0.20
    executable_step_radius: float = 0.04
    coarse_step_radius: float = 0.04
    fine_step_radius: float = 0.02
    ...
    minimum_coarse_steps: int = 8
    minimum_fine_steps: int = 16
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_REALISABILITY_COST_V3_1C"
        and p.seeds == 8
        and p.base_seed == 68726
        and p.adapt_steps == 384
        and p.capability_loss_reduction_fraction == 0.20
        and p.minimum_comparable_seeds == 7
        and p.success_required == 6
        and p.coarse_step_radius == 0.04
        and p.fine_step_radius == 0.02
        and p.minimum_coarse_steps == 8
        and p.minimum_fine_steps == 16
        and p.realizability_ratio_required == 6
        and p.response_retraction_iterations == 2
    )
    if not locked:
        raise ValueError("v3.1c frozen realizability protocol violated")
    return p
...
def one_seed(seed, p, outdir, device, deps):
    ...
    arms = {}
    for label, move_basis, radius in (
        ("moving_coarse", True, p.coarse_step_radius),
        ("moving_fine", True, p.fine_step_radius),
        ("fixed_coarse", False, p.coarse_step_radius),
        ("fixed_fine", False, p.fine_step_radius),
    ):
        arms[label] = execute_online_path(
            "natural_gradient", theta0, basis0, target_response, batches, target_loss, model, spec,
            anchor_x, cap_x, cap_y, metric_x, dev_x, dev_y, source_scales,
            source_row_basis, p, deps, move_basis=move_basis, step_radius=radius,
        )
    return {
        "seed": seed,
...
def summarize(rows, p):
    ...
    counts = {
        "coarse_depth_gate": 0,
        "fine_depth_gate": 0,
        "coarse_fine_moving_action_convergence": 0,
        "coarse_fine_fixed_action_convergence": 0,
    ...
    for row in comparable:
        a = row["arms"]
        mc, mf, fc, ff = a["moving_coarse"], a["moving_fine"], a["fixed_coarse"], a["fixed_fine"]
        counts["coarse_depth_gate"] += mc["steps"] >= p.minimum_coarse_steps
        counts["fine_depth_gate"] += mf["steps"] >= p.minimum_fine_steps
        moving_action_change = abs(mc["moving_fibre_action"] - mf["moving_fibre_action"]) / max(
            abs(mf["moving_fibre_action"]), 1e-12
        )
        fixed_action_change = abs(fc["moving_fibre_action"] - ff["moving_fibre_action"]) / max(
            abs(ff["moving_fibre_action"]), 1e-12
        )
        counts["coarse_fine_moving_action_convergence"] += moving_action_change <= p.coarse_fine_action_relative_gate
        counts["coarse_fine_fixed_action_convergence"] += fixed_action_change <= p.coarse_fine_action_relative_gate
        fisher_ratio = ff["cumulative_retraction_fisher_length"] / max(
            mf["cumulative_retraction_fisher_length"], 1e-12
        )
        tangent_ratio = ff["median_discrete_tangent_residual"] / max(
            mf["median_discrete_tangent_residual"], 1e-12
        )
        ...
        moving_exponent = math.log(
            max(mc["cumulative_retraction_fisher_length"], 1e-12)
            / max(mf["cumulative_retraction_fisher_length"], 1e-12), 2
        )
        fixed_exponent = math.log(
            max(fc["cumulative_retraction_fisher_length"], 1e-12)
            / max(ff["cumulative_retraction_fisher_length"], 1e-12), 2
        )
        ...
        counts["scaling_separation_gate"] += moving_scaling and fixed_scaling
        counts["rowspace_rotation_signal"] += mf["rowspace_rotation_signal"]
        per_seed.append({
            "seed": row["seed"],
            "moving_coarse_steps": mc["steps"],
            "moving_fine_steps": mf["steps"],
            "fixed_coarse_steps": fc["steps"],
            "fixed_fine_steps": ff["steps"],
            "moving_action_coarse_fine_relative_change": moving_action_change,
            "fixed_action_coarse_fine_relative_change": fixed_action_change,
            "fine_fixed_to_moving_retraction_fisher_ratio": fisher_ratio,
            "fine_fixed_to_moving_tangent_residual_ratio": tangent_ratio,
            "moving_retraction_scaling_exponent": moving_exponent,
            "fixed_retraction_scaling_exponent": fixed_exponent,
            "moving_fine_retraction_fisher_length": mf["cumulative_retraction_fisher_length"],
            "fixed_fine_retraction_fisher_length": ff["cumulative_retraction_fisher_length"],
            "moving_fine_retraction_to_tangent_ratio": mf["cumulative_retraction_to_tangent_ratio"],
            "fixed_fine_retraction_to_tangent_ratio": ff["cumulative_retraction_to_tangent_ratio"],
            "moving_fine_max_rowspace_angle_rad": mf["max_rowspace_angle_from_source_rad"],
        })
    n = len(comparable)
    gate = bool(
        n >= p.minimum_comparable_seeds
        and counts["coarse_depth_gate"] >= p.success_required
        and counts["fine_depth_gate"] >= p.success_required
        and counts["coarse_fine_moving_action_convergence"] >= p.success_required
        and counts["coarse_fine_fixed_action_convergence"] >= p.success_required
    ...
    if n < p.minimum_comparable_seeds:
        status = "REALISABILITY_COST_V31C_INADMISSIBLE"
    elif gate:
        status = "REALISABILITY_COST_V31C_SCALING_CANDIDATE_SUPPORTED"
    else:
        status = "REALISABILITY_COST_V31C_SCALING_CANDIDATE_NOT_SUPPORTED"
    return {
        ...
        "per_seed": per_seed,
        "claim_boundary": (
            "Eight-seed development audit comparing online moving-kernel and fixed-source-chart natural "
            "paths at Fisher step radii 0.04 and 0.02. It measures response-retraction Fisher length, "
            ...
            "finite constrained motion. This is not a 16-seed confirmation, exact continuum horizontal lift, arbitrary-path result, "
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_realisability_cost_v3_1c_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Realisability-cost v3.1c seed {index + 1}/{p.seeds}] {seed}", flush=True)
