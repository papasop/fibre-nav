...
PACKAGE = "cner_cnn_mnist_realisability_cost_v3_1c.zip"
ROOT = Path("/content/cner_cnn_mnist_realisability_cost_v3_1c")
OUT = Path("/content/cner_cnn_mnist_realisability_cost_v3_1c_results")
...
print("开始 v3.1c 可实现性成本与步长标度审计；8 seeds、四臂深路径，强烈推荐 A100。")
proc = subprocess.run([
    sys.executable,
    str(ROOT / "cner_cnn_mnist_realisability_cost_v3_1c.py"),
    "--protocol", str(ROOT / "protocol.json"),
    "--output", str(OUT),
    "--no-download",
])
if proc.returncode != 0:
    failure = Path("/content/cner_cnn_mnist_realisability_cost_v3_1c_failure.zip")
    if OUT.exists():
        shutil.make_archive(str(failure.with_suffix("")), "zip", OUT.parent, OUT.name)
        files.download(str(failure))
    raise RuntimeError(f"v3.1c 运行失败，exit={proc.returncode}")
