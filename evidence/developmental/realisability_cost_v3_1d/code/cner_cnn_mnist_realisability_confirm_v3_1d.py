#!/usr/bin/env python3
"""Prospective 16-seed confirmation of moving-fibre realizability scaling."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_REALISABILITY_CONFIRM_V3_1D"
    seeds: int = 16
    base_seed: int = 69726
    adapt_steps: int = 768
    ...
    fixed_retraction_scaling_exponent_max: float = 0.50
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_REALISABILITY_CONFIRM_V3_1D"
        and p.seeds == 16
        and p.base_seed == 69726
    ...
    if not locked:
        raise ValueError("v3.1d frozen prospective protocol violated")
    return p
...
def summarize(rows, p):
    ...
    counts = {
        ...
        "fixed_retraction_nonvanishing_scaling": 0,
        "four_point_fit_quality": 0,
        "exponent_gap_gate": 0,
    ...
    for row in comparable:
        ...
        fixed_exponent, fixed_r2 = loglog_fit(
            p.step_radii, [v["cumulative_retraction_fisher_length"] for v in fixed]
        )
        moving_scaling = moving_exponent >= p.moving_retraction_scaling_exponent_min
        fixed_scaling = fixed_exponent <= p.fixed_retraction_scaling_exponent_max
        fit_quality = moving_r2 >= p.scaling_fit_r2_gate and fixed_r2 >= p.scaling_fit_r2_gate
        ...
        counts["fixed_retraction_nonvanishing_scaling"] += fixed_scaling
        counts["four_point_fit_quality"] += fit_quality
        counts["exponent_gap_gate"] += gap_pass
        counts["scaling_separation_gate"] += moving_scaling and fixed_scaling and fit_quality and gap_pass
    ...
    if n < p.minimum_comparable_seeds:
        status = "REALISABILITY_COST_V31D_INADMISSIBLE"
    elif gate:
        status = "REALISABILITY_COST_V31D_PROSPECTIVE_CONFIRMATION_SUPPORTED"
    else:
        status = "REALISABILITY_COST_V31D_PROSPECTIVE_CONFIRMATION_NOT_SUPPORTED"
    return {
        ...
        "per_seed": per_seed,
        "claim_boundary": (
            "Prospective 16-seed confirmation comparing causal online moving-kernel and fixed-source-chart "
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_realisability_confirm_v3_1d_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Realisability confirmation v3.1d seed {index + 1}/{p.seeds}] {seed}", flush=True)
