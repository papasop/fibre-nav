#!/usr/bin/env python3
"""Four-seed Moving-Fibre F16 Pareto-closure preflight."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_MOVING_FIBRE_F16_QUICK_V3_2A"
    seeds: int = 4
    base_seed: int = 71726
    adapt_steps: int = 384
    capability_loss_reduction_fraction: float = 0.20
    executable_step_radius: float = 0.04
    step_radii: tuple[float, ...] = (0.04,)
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_MOVING_FIBRE_F16_QUICK_V3_2A"
        and p.seeds == 4
        and p.base_seed == 71726
    ...
    if not locked:
        raise ValueError("v3.2a frozen Moving-Fibre F16 preflight protocol violated")
    return p
...
def one_seed(seed, p, outdir, device, deps):
    ...
    for algorithm in p.algorithms:
        arms[algorithm] = execute_online_path(
            algorithm, theta0, basis0, target_response, batches, target_loss, model, spec,
            anchor_x, cap_x, cap_y, metric_x, dev_x, dev_y, source_scales,
            source_row_basis, p, deps, move_basis=True, step_radius=p.executable_step_radius,
        )
    return {
        "seed": seed,
...
def pareto_front(arms):
    ...
    for name in names:
        ...
        for other in names:
            ...
            y = arms[other]
            weak = (
                y["moving_fibre_action"] <= x["moving_fibre_action"]
                and y["cumulative_retraction_fisher_length"] <= x["cumulative_retraction_fisher_length"]
            )
            strict = (
                y["moving_fibre_action"] < x["moving_fibre_action"]
                or y["cumulative_retraction_fisher_length"] < x["cumulative_retraction_fisher_length"]
            )
            if weak and strict:
                dominated = True
                break
...
def summarize(rows, p):
    ...
    counts = {
        "natural_action_minimum": 0,
        "natural_retraction_cost_minimum": 0,
        "natural_pareto_nondominated": 0,
        "natural_action_beats_wrong_natural": 0,
    ...
    for row in comparable:
        arms = row["arms"]
        action_winner = min(arms, key=lambda n: arms[n]["moving_fibre_action"])
        cost_winner = min(arms, key=lambda n: arms[n]["cumulative_retraction_fisher_length"])
        wrong_action_winner = min(arms, key=lambda n: arms[n]["wrong_moving_fibre_action"])
        front = pareto_front(arms)
        nat = arms["natural_gradient"]
        wrong = arms["wrong_fisher_natural_gradient"]
        beats_wrong_action = nat["moving_fibre_action"] < wrong["moving_fibre_action"]
        jointly_dominates_wrong = (
            nat["moving_fibre_action"] <= wrong["moving_fibre_action"]
            and nat["cumulative_retraction_fisher_length"] <= wrong["cumulative_retraction_fisher_length"]
            and (
                nat["moving_fibre_action"] < wrong["moving_fibre_action"]
                or nat["cumulative_retraction_fisher_length"] < wrong["cumulative_retraction_fisher_length"]
            )
        )
        counts["natural_action_minimum"] += action_winner == "natural_gradient"
        counts["natural_retraction_cost_minimum"] += cost_winner == "natural_gradient"
        counts["natural_pareto_nondominated"] += "natural_gradient" in front
        counts["natural_action_beats_wrong_natural"] += beats_wrong_action
        counts["natural_jointly_dominates_wrong_natural"] += jointly_dominates_wrong
        counts["natural_wins_under_wrong_metric"] += wrong_action_winner == "natural_gradient"
        counts["natural_rowspace_rotation_signal"] += nat["rowspace_rotation_signal"]
        per_seed.append({
            "seed": row["seed"],
            "action_winner": action_winner,
            "retraction_cost_winner": cost_winner,
            "wrong_metric_action_winner": wrong_action_winner,
            "pareto_front": front,
            "natural_action": nat["moving_fibre_action"],
            "natural_retraction_fisher_cost": nat["cumulative_retraction_fisher_length"],
            "wrong_natural_action": wrong["moving_fibre_action"],
            "wrong_natural_retraction_fisher_cost": wrong["cumulative_retraction_fisher_length"],
            "natural_action_minus_wrong": nat["moving_fibre_action"] - wrong["moving_fibre_action"],
            "natural_cost_minus_wrong": (
                nat["cumulative_retraction_fisher_length"]
                - wrong["cumulative_retraction_fisher_length"]
            ),
            "arms": {
                name: {
                    "moving_fibre_action": value["moving_fibre_action"],
                    "retraction_fisher_cost": value["cumulative_retraction_fisher_length"],
                    "moving_fisher_length": value["moving_fisher_length"],
                    "steps": value["steps"],
                }
    ...
    gate = bool(
        ...
        and counts["natural_pareto_nondominated"] >= p.success_required
        and counts["natural_action_minimum"] >= p.success_required
        and counts["natural_action_beats_wrong_natural"] >= p.success_required
        and counts["natural_wins_under_wrong_metric"] <= 1
        and counts["natural_rowspace_rotation_signal"] >= p.success_required
    )
    if n < p.minimum_comparable_seeds:
        status = "MOVING_FIBRE_F16_V32A_INADMISSIBLE"
    elif gate:
        status = "MOVING_FIBRE_F16_V32A_PARETO_CLOSURE_CANDIDATE_SUPPORTED"
    else:
        status = "MOVING_FIBRE_F16_V32A_PARETO_CLOSURE_CANDIDATE_NOT_SUPPORTED"
    return {
        ...
        "counts": counts,
        "moving_fibre_f16_candidate_gate": gate,
        "excluded": exclusions,
        "per_seed": per_seed,
        "claim_boundary": (
            "Four-seed developmental preflight joining v3.1e admissibility and F16 ordering. Six named "
            "algorithms are generated causally in a transported eight-dimensional chart projected into the "
            "current response kernel at every step, with a common 20% capability endpoint. The two primary "
            "axes are moving capacity-normalized Fisher action and response-retraction Fisher cost; no fitted "
            "lambda or scalar composite is used. A pass is a Pareto-closure candidate only, not 16-seed "
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_moving_fibre_f16_quick_v3_2a_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Moving-Fibre F16 v3.2a seed {index + 1}/{p.seeds}] {seed}", flush=True)
