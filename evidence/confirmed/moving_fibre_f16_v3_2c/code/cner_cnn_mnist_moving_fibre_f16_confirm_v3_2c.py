#!/usr/bin/env python3
"""Sixteen-seed prospective confirmation of Moving-Fibre F16 scaling."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_MOVING_FIBRE_F16_CONFIRM_V3_2C"
    seeds: int = 16
    base_seed: int = 73726
    adapt_steps: int = 768
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_MOVING_FIBRE_F16_CONFIRM_V3_2C"
        and p.seeds == 16
        and p.base_seed == 73726
    ...
    if not locked:
        raise ValueError("v3.2c frozen Moving-Fibre F16 confirmation protocol violated")
    return p
...
def summarize(rows, p):
    ...
    counts = {
        ...
        "natural_smallest_radius_cost_persistently_higher": 0,
    }
    per_seed = []
    for row in comparable:
        ...
        counts["natural_smallest_radius_cost_near_minimum"] += natural_cost_ratios[-1] <= 1.25
        counts["natural_smallest_radius_cost_persistently_higher"] += natural_cost_ratios[-1] >= 1.50
        per_seed.append({
            "seed": row["seed"],
    ...
    gate = bool(
        ...
        and counts["natural_wins_under_wrong_metric_at_any_radius"] <= p.wrong_metric_natural_win_ceiling
        and counts["natural_rowspace_rotation_signal"] >= p.success_required
    )
    if n < p.minimum_comparable_seeds:
        status = "MOVING_FIBRE_F16_V32C_INADMISSIBLE"
    elif gate:
        status = "MOVING_FIBRE_F16_V32C_PROSPECTIVE_CONFIRMATION_SUPPORTED"
    else:
        status = "MOVING_FIBRE_F16_V32C_PROSPECTIVE_CONFIRMATION_NOT_SUPPORTED"
    return {
        ...
        "per_seed": per_seed,
        "claim_boundary": (
            "Sixteen-seed, four-radius prospective confirmation testing whether the higher finite-step retraction cost "
            ...
            "No fitted lambda or scalar composite is used. A pass prospectively confirms the restricted six-algorithm "
            "Moving-Fibre F16 scaling result in this frozen CNN--MNIST model; it is not complete-kernel optimization, "
            "continuum geometry, local stationarity, GPT-2 transfer, "
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_moving_fibre_f16_confirm_v3_2c_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Moving-Fibre F16 confirmation v3.2c seed {index + 1}/{p.seeds}] {seed}", flush=True)
