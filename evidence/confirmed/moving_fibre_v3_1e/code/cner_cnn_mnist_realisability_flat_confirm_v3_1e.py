#!/usr/bin/env python3
"""Prospective zero-slope-aware confirmation of moving-fibre realizability scaling."""
...
class Protocol(E.Protocol):
    protocol_name: str = "CNER_CNN_MNIST_REALISABILITY_FLAT_CONFIRM_V3_1E"
    seeds: int = 16
    base_seed: int = 70726
    adapt_steps: int = 768
    ...
    realizability_ratio_required: int = 12
    ...
    scaling_separation_required: int = 12
...
def load_protocol(path):
    ...
    if path:
        ...
        p = Protocol(**raw)
    locked = (
        p.protocol_name == "CNER_CNN_MNIST_REALISABILITY_FLAT_CONFIRM_V3_1E"
        and p.seeds == 16
        and p.base_seed == 70726
        and p.adapt_steps == 768
        and p.capability_loss_reduction_fraction == 0.20
        and p.minimum_comparable_seeds == 14
        and p.success_required == 12
        and tuple(p.step_radii) == (0.08, 0.04, 0.02, 0.01)
        and p.minimum_largest_radius_steps == 4
        and p.minimum_smallest_radius_steps == 24
        and p.realizability_ratio_required == 12
        and p.response_retraction_iterations == 2
        and p.fixed_retraction_abs_exponent_max == 0.25
        and p.fixed_cost_relative_span_max == 0.20
    )
    if not locked:
        raise ValueError("v3.1e frozen zero-slope-aware protocol violated")
    return p
...
def one_seed(seed, p, outdir, device, deps):
    ...
    arms = {}
    for mode, move_basis in (("moving", True), ("fixed", False)):
        for index, radius in enumerate(p.step_radii):
            label = f"{mode}_r{index}"
            arms[label] = execute_online_path(
                "natural_gradient", theta0, basis0, target_response, batches, target_loss, model, spec,
                anchor_x, cap_x, cap_y, metric_x, dev_x, dev_y, source_scales,
                source_row_basis, p, deps, move_basis=move_basis, step_radius=float(radius),
            )
    return {
        "seed": seed,
...
def summarize(rows, p):
    ...
    counts = {
        "largest_radius_depth_gate": 0,
        "smallest_radius_depth_gate": 0,
        "small_step_moving_action_convergence": 0,
        "small_step_fixed_action_convergence": 0,
        "fixed_fisher_retraction_at_least_2x_moving": 0,
        "fixed_tangent_residual_at_least_2x_moving": 0,
        "joint_realisability_ratio_gate": 0,
        "moving_retraction_positive_scaling": 0,
        "fixed_retraction_near_zero_exponent": 0,
        "fixed_cost_flatness": 0,
        "moving_four_point_fit_quality": 0,
        "exponent_gap_gate": 0,
        "scaling_separation_gate": 0,
        "rowspace_rotation_signal": 0,
    }
    per_seed = []
    for row in comparable:
        a = row["arms"]
        moving = [a[f"moving_r{i}"] for i in range(len(p.step_radii))]
        fixed = [a[f"fixed_r{i}"] for i in range(len(p.step_radii))]
        counts["largest_radius_depth_gate"] += moving[0]["steps"] >= p.minimum_largest_radius_steps
        counts["smallest_radius_depth_gate"] += moving[-1]["steps"] >= p.minimum_smallest_radius_steps
        moving_action_change = abs(moving[-2]["moving_fibre_action"] - moving[-1]["moving_fibre_action"]) / max(
            abs(moving[-1]["moving_fibre_action"]), 1e-12
        )
        fixed_action_change = abs(fixed[-2]["moving_fibre_action"] - fixed[-1]["moving_fibre_action"]) / max(
            abs(fixed[-1]["moving_fibre_action"]), 1e-12
        )
        counts["small_step_moving_action_convergence"] += moving_action_change <= p.coarse_fine_action_relative_gate
        counts["small_step_fixed_action_convergence"] += fixed_action_change <= p.coarse_fine_action_relative_gate
        fisher_ratio = fixed[-1]["cumulative_retraction_fisher_length"] / max(
            moving[-1]["cumulative_retraction_fisher_length"], 1e-12
        )
        tangent_ratio = fixed[-1]["median_discrete_tangent_residual"] / max(
            moving[-1]["median_discrete_tangent_residual"], 1e-12
        )
        fisher_pass = fisher_ratio >= p.fixed_to_moving_fisher_retraction_ratio_gate
        tangent_pass = tangent_ratio >= p.fixed_to_moving_tangent_residual_ratio_gate
        counts["fixed_fisher_retraction_at_least_2x_moving"] += fisher_pass
        counts["fixed_tangent_residual_at_least_2x_moving"] += tangent_pass
        counts["joint_realisability_ratio_gate"] += fisher_pass and tangent_pass
        moving_exponent, moving_r2 = loglog_fit(
            p.step_radii, [v["cumulative_retraction_fisher_length"] for v in moving]
        )
        fixed_costs = [v["cumulative_retraction_fisher_length"] for v in fixed]
        fixed_exponent, fixed_r2 = loglog_fit(p.step_radii, fixed_costs)
        moving_scaling = moving_exponent >= p.moving_retraction_scaling_exponent_min
        fixed_scaling = abs(fixed_exponent) <= p.fixed_retraction_abs_exponent_max
        fixed_relative_span = (max(fixed_costs) - min(fixed_costs)) / max(float(np.median(fixed_costs)), 1e-12)
        fixed_flatness = fixed_relative_span <= p.fixed_cost_relative_span_max
        moving_fit_quality = moving_r2 >= p.scaling_fit_r2_gate
        exponent_gap = moving_exponent - fixed_exponent
        gap_pass = exponent_gap >= p.fixed_minus_moving_exponent_gap
        counts["moving_retraction_positive_scaling"] += moving_scaling
        counts["fixed_retraction_near_zero_exponent"] += fixed_scaling
        counts["fixed_cost_flatness"] += fixed_flatness
        counts["moving_four_point_fit_quality"] += moving_fit_quality
        counts["exponent_gap_gate"] += gap_pass
        counts["scaling_separation_gate"] += (
            moving_scaling and fixed_scaling and fixed_flatness and moving_fit_quality and gap_pass
        )
        counts["rowspace_rotation_signal"] += moving[-1]["rowspace_rotation_signal"]
        per_seed.append({
            "seed": row["seed"],
            "step_radii": list(p.step_radii),
            "moving_steps": [v["steps"] for v in moving],
            "fixed_steps": [v["steps"] for v in fixed],
            "moving_small_step_action_relative_change": moving_action_change,
            "fixed_small_step_action_relative_change": fixed_action_change,
            "smallest_radius_fixed_to_moving_retraction_fisher_ratio": fisher_ratio,
            "smallest_radius_fixed_to_moving_tangent_residual_ratio": tangent_ratio,
            "moving_retraction_scaling_exponent": moving_exponent,
            "fixed_retraction_scaling_exponent": fixed_exponent,
            "moving_scaling_r2": moving_r2,
            "fixed_scaling_r2": fixed_r2,
            "fixed_cost_relative_span": fixed_relative_span,
            "moving_minus_fixed_exponent": exponent_gap,
            "moving_retraction_fisher_lengths": [v["cumulative_retraction_fisher_length"] for v in moving],
            "fixed_retraction_fisher_lengths": [v["cumulative_retraction_fisher_length"] for v in fixed],
            "moving_smallest_radius_max_rowspace_angle_rad": moving[-1]["max_rowspace_angle_from_source_rad"],
        })
    n = len(comparable)
    gate = bool(
        n >= p.minimum_comparable_seeds
        and counts["largest_radius_depth_gate"] >= p.success_required
        and counts["smallest_radius_depth_gate"] >= p.success_required
        and counts["small_step_moving_action_convergence"] >= p.success_required
        and counts["small_step_fixed_action_convergence"] >= p.success_required
        and counts["joint_realisability_ratio_gate"] >= p.realizability_ratio_required
        and counts["scaling_separation_gate"] >= p.scaling_separation_required
        and counts["rowspace_rotation_signal"] >= p.success_required
    )
    if n < p.minimum_comparable_seeds:
        status = "REALISABILITY_COST_V31E_INADMISSIBLE"
    elif gate:
        status = "REALISABILITY_COST_V31E_ZERO_SLOPE_CONFIRMATION_SUPPORTED"
    else:
        status = "REALISABILITY_COST_V31E_ZERO_SLOPE_CONFIRMATION_NOT_SUPPORTED"
    return {
        ...
        "counts": counts,
        "realisability_cost_scaling_gate": gate,
        "excluded": exclusions,
        "per_seed": per_seed,
        "claim_boundary": (
            "Prospective 16-seed zero-slope-aware confirmation comparing causal online moving-kernel and fixed-source-chart "
            "natural paths at four frozen Fisher step radii 0.08, 0.04, 0.02, and 0.01. It measures response-retraction Fisher length, "
            "tangent residual, action convergence, and step-size scaling at one matched 20% endpoint. "
            "No post-hoc lambda-weighted composite action is tested. Newton response retraction approximates "
            "finite constrained motion. This is not an exact continuum horizontal lift, arbitrary-path result, "
            "GPT-2 transfer, stationarity theorem, or global variational law. The moving arm alone is required "
            "to have high log-log R-squared; the theoretically constant fixed arm is tested by absolute exponent "
            "and relative span, frozen before these new seeds are observed."
        ),
    }
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("cner_cnn_mnist_realisability_flat_confirm_v3_1e_results"))
    ...
    for index in range(p.seeds):
        ...
        print(f"[Realisability flat confirmation v3.1e seed {index + 1}/{p.seeds}] {seed}", flush=True)
    ...
    (args.output / "REPORT.md").write_text(
        "# Moving-fibre realizability-cost zero-slope-aware confirmation v3.1e\n\n```json\n"
