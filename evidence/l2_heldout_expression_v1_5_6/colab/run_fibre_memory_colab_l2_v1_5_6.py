#!/usr/bin/env python3
"""Robust Colab launcher for GPT-2 Fibre Memory L2 v1.5.6 one-seed audit."""
import argparse
import os
import shutil
import subprocess
...
def choose_zip(requested, explicit):
    ...
    try:
        from google.colab import files
        print("Please select gpt2_fibre_memory_l2_v1_5_6.zip")
...
def main():
    ...
    p.add_argument("--zip", default="/content/gpt2_fibre_memory_l2_v1_5_6.zip")
    p.add_argument("--workdir", default="/content/fibre_memory_l2_run_v1_5_6")
    ...
    print("[L2 v1.5.6 worst-margin smoke] frozen seed: 81801", flush=True)
    code = stream(cmd, root, log_path)

    archive = Path(shutil.make_archive("/content/fibre_memory_l2_results_v1_5_6", "zip", results))
    if code != 0:
