#!/usr/bin/env python3
"""Robust Colab launcher for Moving Fibre Intelligence L3 v1.7.8."""
import argparse
import os
import shutil
import subprocess
...
def choose_zip(requested, explicit):
    ...
    try:
        from google.colab import files
        print("Please select moving_fibre_intelligence_l3_v1_7_8.zip")
...
def main():
    ...
    p.add_argument("--zip", default="/content/moving_fibre_intelligence_l3_v1_7_8.zip")
    p.add_argument("--workdir", default="/content/moving_fibre_intelligence_l3_run_v1_7_8")
    ...
    print("[MFI L3 v1.7.8 mixed-slot margin repair] frozen seed: 81902", flush=True)
    code = stream(cmd, root, log_path)

    archive = Path(shutil.make_archive("/content/moving_fibre_intelligence_l3_results_v1_7_8", "zip", results))
    if code != 0:
