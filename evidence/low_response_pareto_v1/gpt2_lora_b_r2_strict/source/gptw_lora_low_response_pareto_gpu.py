#!/usr/bin/env python3
"""CUDA primitives for the frozen GPT-2 native LoRA-B Pareto audit."""
...
def permuted_row(row,seed):
    g=torch.Generator(device=row.device).manual_seed(seed);p=torch.randperm(row.shape[1],generator=g,device=row.device);sign=torch.where(torch.rand(row.shape[1],generator=g,device=row.device)<.5,-1.,1.).double()
    return row[:,p]*sign
...
def auc(values):
    ...
    return float(integrate(values,BUDGETS)/(BUDGETS[-1]-BUDGETS[0]))
