#!/usr/bin/env python3
"""One-click Colab CPU launcher for CNER v4.6 Pareto audit."""
from pathlib import Path
import hashlib,shutil,subprocess,sys,zipfile
ROOT=Path("/content") if Path("/content").exists() else Path.cwd()
print("="*78);print("CNER v4.6 low-response Pareto frontier — CPU 8 seeds");print("No GPU required.");print("="*78)
try:from google.colab import files
except Exception as exc:raise RuntimeError("Run inside Google Colab.") from exc
print("Upload cner_v4_6_cpu_low_response_pareto.zip");uploaded=files.upload()
if len(uploaded)!=1:raise RuntimeError("Upload exactly one source ZIP.")
name,payload=next(iter(uploaded.items()))
if not name.lower().endswith(".zip") or "v4_6" not in name.lower():raise RuntimeError(f"Unexpected package: {name}")
source=ROOT/name;source.write_bytes(payload);run=ROOT/"cner_v4_6_cpu_run"
...
output=ROOT/"cner_v4_6_cpu_pareto_results"
subprocess.check_call([sys.executable,"-u",str(run/"cner_v4_6_cpu_pareto_frontier.py"),"--output",str(output)])
