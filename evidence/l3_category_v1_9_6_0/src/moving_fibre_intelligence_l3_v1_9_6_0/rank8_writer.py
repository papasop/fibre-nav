#!/usr/bin/env python3
"""Moving Fibre Intelligence L3 v1.7.8 mixed-slot margin repair audit."""
...
def run_write(model, tok, params, cfg, anchors, response_ids, response0, logits0,
    ...
    # Non-interventional endpoint diagnostic: compare the two slot gradients
    # using declared training prompts only. It neither changes the endpoint nor
    # participates in checkpoint selection.
    slot_gradient_cosine = None
    slot_gradient_gram_condition = None
    if len(exact_prompts) == 2 and len(write_prompts) % 2 == 0:
        slot_gradients = []
        for offset in (0, 1):
            slot_loss = semantic_memory_loss(
                model, tok, write_prompts[offset::2], write_targets[offset::2],
                candidate_ids, device, 1, cfg["semantic_pull_weight"],
                cfg["semantic_consistency_weight"], cfg["worst_item_weight"],
                cfg["worst_item_temperature"], cfg["margin_weight"],
                cfg["margin_target"], False, 0.0, 0.0)[0]
            slot_gradients.append(flat(torch.autograd.grad(slot_loss, params)))
        r_diag = response_vector(model, tok, anchors, response_ids, device)
        J_diag = jacobian_rows(r_diag, params)
        tangent = [kernel_project(g, J_diag) for g in slot_gradients]
        tangent = [g / (g.norm() + 1e-12) for g in tangent]
        slot_gradient_cosine = float(torch.dot(tangent[0], tangent[1]).detach().item())
        gram_diag = torch.stack(tangent) @ torch.stack(tangent).T
        eig = torch.linalg.eigvalsh(gram_diag)
        slot_gradient_gram_condition = float(
            (eig.max() / eig.clamp_min(1e-12).min()).detach().item())
    return {"exact_accuracy":read_accuracy(model,tok,exact_prompts,exact_eval_targets,candidate_ids,device),
            ...
            "endpoint_slot_gradient_cosine":slot_gradient_cosine,
            "endpoint_slot_gradient_gram_condition":slot_gradient_gram_condition,
...
def main():
    ...
    for seed in cfg["seeds"]:
        ...
        records.append(one_seed_l3_writer(cfg,seed,out))
