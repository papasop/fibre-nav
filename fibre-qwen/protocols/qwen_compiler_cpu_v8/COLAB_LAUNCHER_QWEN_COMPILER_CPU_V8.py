#!/usr/bin/env python3
"""V8 CPU Colab uploader / local launcher. Paste into a cell or run as a .py."""
import argparse
import hashlib
import json
import os
...
PROFILE = 'compiled_interface'
MINUTES = 90
THREADS = 4
USE_GOOGLE_DRIVE = False
AUTO_DOWNLOAD_STAGES = True
AUTO_DOWNLOAD_FINAL = True
SOURCE_MANIFEST_SHA256 = '7be2e646057b24b5b2fe4576af6ec7046ee30e5c894a35579c7679bcd4187bbe'
PROTOCOL_ID = 'QWEN_COMPILER_CPU_V8'
    ...
    with zipfile.ZipFile(path) as z:
        ...
        if manifests:
            ...
            if lock.get('protocol_id')!=PROTOCOL_ID or lock.get('source_manifest_sha256')!=SOURCE_MANIFEST_SHA256:
                raise ValueError('只能续跑本次 V8 的完整或阶段 results.zip；旧版本结果不能混入。')
        ...
        if len(candidates)!=1:raise ValueError('未找到唯一实验源；请上传 qwen_compiler_cpu_v8.zip。')
        manifest,prefix=candidates[0];m=json.loads(z.read(manifest))
        if sha(canonical(m))!=SOURCE_MANIFEST_SHA256:raise ValueError('V8 源清单不匹配；启动器与 ZIP 必须来自同一次交付。')
        for n,h in m.items():
            safe_name(n)
...
def prepare_dependencies(root,source,skip_install=False):
    ...
    shared=Path('/content/qwen_scope_cpu_v2/cpu_dependencies')
    if shared.is_dir() and not deps.exists():deps=shared
...
def launch(argv=None):
    ...
    parser.add_argument('--workdir',type=Path,default=Path('/content/qwen_compiler_cpu_v8') if colab else Path.cwd()/'qwen_compiler_cpu_v8_work')
    ...
    root=a.workdir.resolve();root.mkdir(parents=True,exist_ok=True)
    print('CPU 模式，GPU 不参与计算。Profile: '+PROFILE,flush=True)
    print('Qwen compiler V8：声明语法解析＋冻结模板，128个模型检查点，仅受限生成；纯CPU，无训练。',flush=True)
    candidates=[p for p in (a.source,a.resume) if p]
    source=root/'source';out=root/'run'/'cpu_compiled_interface'
    if not candidates and colab:
        print('请上传 qwen_compiler_cpu_v8.zip；续跑可同时上传一份 V8 *_results.zip。',flush=True)
    ...
    if not (source/'MANIFEST.json').is_file():raise RuntimeError('请提供 V8 源 ZIP。它已包含历史对照，不需要之前版本的结果。')
    ...
    if USE_GOOGLE_DRIVE and colab:
        try:
            ...
            drive_target=Path('/content/drive/MyDrive/qwen_compiler_cpu_v8');drive_target.mkdir(parents=True,exist_ok=True)
    ...
    os.environ['V8_CPU_DEPS']=str(deps.resolve())
    ...
    log=root/'QWEN_COMPILER_V8_SESSION_OUTPUT.txt';child=None
    try:
