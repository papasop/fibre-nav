...
def main():
 ...
 def select(q):
  z=sorted(((sum(k.casefold() in q.casefold() for k in c["keywords"]),i,c) for i,c in enumerate(cards)),key=lambda x:(-x[0],x[1]));return [x[2] for x in z if x[0]>0][:2] or [cards[-1]]
 def system(arm,r):
  if arm=="bare":return "",[]
  if arm=="long_constitution":return long,[]
  chosen=select(r["prompt"]);text="你是严谨研究助手。只使用与当前问题有关的规则，不要复述无关术语。先直接回答，再给一句边界。\n"+"\n".join(f"规则：{c['rule']}\n正确范式：{c['positive']}\n禁止范式：{c['negative']}" for c in chosen);return text,[c["id"] for c in chosen]
 def pre(arm,r):
