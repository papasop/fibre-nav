...
from repair_kl import QwenKLChart, KLAwareController
from evaluate_tasks import evaluate_stage, summarize, PROTOCOL as TASK_PROTOCOL

PROTOCOL = {
    'id':'MFI_KL_DIRECTION_REPAIR_001', 'model':MODEL, 'revision':REVISION,
    'development_seeds':[84031,84047,84061], 'heldout_seeds':[84103,84119,84137],
    'algorithms':['original','kl_aware'], 'budget':asdict(Budget()),
    'repair':{'activation_fraction':0.5,'inward_fraction':0.25},
...
def main():
    ...
    files=['core.py','qwen_l1.py','repair_kl.py','verify_repair.py','evaluate_tasks.py']
    hashes={}
    for n in files:
    ...
    repaired=[r for r in runs if r['algorithm']=='kl_aware']
