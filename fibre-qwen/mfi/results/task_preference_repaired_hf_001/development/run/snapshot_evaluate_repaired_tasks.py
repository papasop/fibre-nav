...
from repair_response import ResponseKLController as MFIController
from qwen_l1 import MODEL, REVISION
from repair_kl import QwenKLChart as QwenChart
...
PROTOCOL = {
    'id': 'MFI_REPAIRED_PREFERENCE_HF_001', 'model': MODEL, 'revision': REVISION,
    'writer': 'ResponseKLController', 'writer_source_commit': '50d8da2d7ec775af8337c73217b2be28ae0ca711', 'seeds': SEEDS, 'budget': asdict(Budget()), 'tasks': TASKS,
...
def main():
    ...
    parser.add_argument('--new-seeds', action='store_true')
    a = parser.parse_args()
    SEEDS[:] = [84301,84317,84329] if a.new_seeds else [84031,84047,84061]
    PROTOCOL['cohort'] = 'previously_validated_new_seeds' if a.new_seeds else 'development'
    a.out.mkdir(parents=True, exist_ok=False)
    (a.out/'protocol.json').write_text(json.dumps(PROTOCOL, indent=2))
    names = ['evaluate_repaired_tasks.py', 'evaluate_tasks.py', 'qwen_l1.py', 'core.py', 'repair_kl.py', 'repair_response.py']
